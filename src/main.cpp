    ...
    std::shared_ptr<std::string const> const& doc_root)
{
    ...
    int length = socket.read_some(boost::asio::buffer(buff, 1024), ec);
    std::cout << std::string(buff, length) << " " << length << std::endl;
    boost::system::error_code ignored_error;
    boost::asio::write(socket, boost::asio::buffer(std::string("HTTP/1.0 200 Connection established\r\nProxy-agent: node.js-proxy\r\n\r\n")), ignored_error);
    // length = socket.read_some(boost::asio::buffer(buff, 1024), ec);
